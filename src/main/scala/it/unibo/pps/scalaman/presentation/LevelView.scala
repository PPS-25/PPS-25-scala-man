...
final case class LevelView(
    player: RenderedMovement, enemies: Vector[RenderedEnemyMovement], collectibles: Set[Collectible],
    remaining: Int, lives: Int, applied: Set[BonusEffect], status: GameState, score: Int,
    elapsed: FiniteDuration, timeLeft: Option[FiniteDuration], mode: LeaderboardMode = LeaderboardMode.Classic
)

object LevelView:
  def of(level: LevelState): LevelView = LevelView(
    RenderedMovement.of(level.player),
    level.enemies.map(enemy => RenderedEnemyMovement(RenderedMovement.of(enemy.entity), enemy.kind)),
    level.collectibles.placed, level.collectibles.remaining, level.progress.lives,
    level.effects.active(level.clock.elapsed), level.status, level.liveScore,
    level.clock.elapsed.toSeconds.seconds, level.mode.timeLeft(level.clock).map(wholeSecondsUp),
...
object RenderedMovement:
  def of(entity: MovingEntity): RenderedMovement = entity.movement match
    case Some(movement) => RenderedMovement(movement.from, movement.to, 1 - (movement.remaining / entity.timePerPos), entity.facing)
