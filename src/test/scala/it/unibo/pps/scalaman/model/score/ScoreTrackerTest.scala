...
import it.unibo.pps.scalaman.model.score.ScoringEvent.{BasicItem, BonusItem, EnemyKill, RemainingLives}
...
class ScoreTrackerTest extends AnyFunSuite:
  ...
  test("every event awards the points defined by its scoring rule"){
    ...
    expectedPoints.foreach{
      case (event, points) => assert(ScoringRule.awardedPoints(event) == points)
    }
  }
  ...
  test("an enemy kill awards 200 points, doubled at every combo increase"){
    ...
    expectedPoints.foreach{
      case (combo, points) => assert(ScoringRule.awardedPoints(EnemyKill, combo) == points)
    }
  }

  test("resetting a combo brings its total back to 0"){
    assert(ScoreTracker(combo = 3)
      .resetCombo
      .combo == 0)
  }
