...
\begin{itemize}
  ...
  given the lives the player has left, what is left to gather, and the time eleapsed. A mode also
  ...
  the game ends onlu when the player runs out of lives.
...
\begin{itemize}
  ...
  exactly one cell, faces one direction, and may be part-way through a movement towards and adjacent cell.
  ...
  \item \textbf{Fame clock}: the time elapsed since the game began, advanced by the time passed between one updated
...
\begin{enumerate}
  ...
  or to opne one thay have written themselves from a file of their own.
...
\begin{itemize}
  ...
  and only the files whore rows are all of the same, non-zero length.
  ...
  \item \textbf{Grand and expire effects}: gathering a bonus item must grant its effect for a fixed time. An effect
...
through a test, that shows that a full update takes about 350 microseconds, that leaver abundant margin.
...
\begin{enumerate}
  ...
  reachable through an interface describing what is stored rather than though the direct use of the file
