...
\begin{itemize}
  \item \textbf{Accessible arcade game}: Scala-Man is a single-player maze game, distributed as a runnable executable
  and played locally. Its maze clearing, enemy pursuit, and power-up mechanics must be immediately recognisable.
  \item \textbf{Fast start}: a player chooses a name, maze, and game mode with little or no need for documentation.
  Each mode offers a distinct objective, allowing the same maze to be played in more than one way.
  \item \textbf{Continuity and comparison}: games can be saved and resumed, while completed games are recorded so runs
  on the same maze and in the same mode remain comparable.
  \item \textbf{Player-authored content}: players can play both supplied mazes and mazes they have written themselves.
  \item \textbf{Extensibility}: new game modes, enemy behaviours, and bonus effects can be added after delivery.
  \item \textbf{Project constraints}: development is limited to four one-week sprints of roughly fifteen hours per
  member and uses functional programming, systematic automated testing, and an agile process.
\end{itemize}
