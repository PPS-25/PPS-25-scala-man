...
\begin{itemize}
  ...
  selected mode: Classic scores collected standard items, collected bonus items, defeated enemies, and lives left at
  the end; Timed scores the seconds remaining when the game ends; Survival scores the difficulty waves survived.
  ...
  for each second remaining when the maze is cleared, and nothing else is scored. In Survival, a point is awarded
  for each difficulty wave survived.
...
\begin{center}
  \begin{tabular}{|p{3cm}|p{8.8cm}|}
    ...
    \hline
    Responsiveness & A complete model update must fit within the 16 milliseconds a frame is allotted at 60 frames per
    second, on every maze supplied with the game, so that the model is never the reason a frame is missed. The measurements
    and its results are reported in the chapter on testing.\\
    \hline
