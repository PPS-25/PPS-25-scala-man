...
The design distinguishes invalid data arriving from outside the game from an invalid state created by the game
itself. The former is expected and is represented explicitly: parsing and validation return an \texttt{Either} whose
left side contains domain errors, while absence that is normal, such as an unavailable next move, is represented by
\texttt{Option}. This keeps malformed map files and save files out of the playing model without using exceptions for
ordinary input failures.
...
\texttt{LevelState} is the aggregate at the centre of the model. It brings together a validated maze, the player and
...
\texttt{ActiveEffects} models a temporary bonus as an effect mapped to its absolute expiration instant. Asking which
...
Enemy variation is isolated behind \texttt{EnemyMovementStrategy}. A strategy receives an immutable
...
The AI stage is responsible for orchestration rather than decision logic. It selects a strategy from the enemy kind,
builds the context only for idle enemies, records any target the strategy wants to retain, and turns a chosen position
into an entity movement. Hunter, Anticipator, and Patroller thus vary independently while sharing the same scheduling
rule. Common navigation lives in \texttt{EnemyMovement}: breadth-first search finds shortest paths through walkable
cells and teleport links, and a fixed neighbour order makes equal paths deterministic. An enemy that has just left a
teleport cannot immediately use it again, preventing a bounce between its two ends.

\subsection{Game modes}

\texttt{GameMode} is a domain policy rather than a flag checked throughout the level. Its operations decide the
...
The mutable-looking phrase ``game tick'' is represented by \texttt{GameStateUpdatePipeline[S]}, a value holding an
...
Reading a maze is a boundary pipeline with four deliberately distinct representations. \texttt{MapLoader} performs
...
The view never receives \texttt{LevelState}. \texttt{LevelView.of} projects it to the immutable data needed for
...
\texttt{Screen} is a small sum type derived from loop state, level outcome, and the pre-game countdown. It separates
...
\texttt{Application} is the immutable controller aggregate. It holds the environment port, mode tuning, optional
...
new projection differs from the previous one. It returns an updated rendering value, so listener registration and the
last visible snapshot remain explicit instead of hiding mutable observer state.
...
The adapters are divided by the external format they understand. \texttt{GameFilesEnvironment} owns the user-data
